...
def construct_svhn_xnornet(input_var, alpha, eps):
    # input layer
...
def init_model_params(net, snapshot):
    print('Initializing the model parameters...')
    with np.load(snapshot) as mf:
...
if __name__=='__main__':
    ...
    # Model file name
    model_file = 'xnor_net_svhn_nonxnor_first_lyr.npz'
    ...
    net = construct_svhn_xnornet(input_vars, alpha, eps)

    # Load data
    print('Loading the data...')
    train_x, val_x, test_x, train_y, val_y, test_y = cnn_utils.load_data('svhn')
    ...
    if resume and os.path.exists(model_file):
        init_model_params(net, model_file)
        LR_start = 0.00032
        no_epochs = 165
        LR_decay = (LR_end/LR_start)**(1./no_epochs)
        print('LR_start = {:f}\tLR_end = {:f}\tLR_decay = {:f}'.format(LR_start, LR_end, LR_decay))
